"""
One-time setup: creates the Kafka topics the Discovery worker needs.
...
Does NOT touch userSession.notifications — that already exists from
the DFV project setup and is shared between both workers.
...
TOPICS = [
    NewTopic(name="userSession.discovery", num_partitions=3, replication_factor=1),
    NewTopic(name="userSession.discovery.dlq", num_partitions=1, replication_factor=1),
]
