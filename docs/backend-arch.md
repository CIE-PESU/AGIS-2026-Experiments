...
8. [RBAC](#8-rbac)
9. [MongoDB Design](#9-mongodb-design)
10. [Repository Layer](#10-repository-layer)
11. [Service Layer](#11-service-layer)
12. [Kafka Architecture](#12-kafka-architecture)
13. [Worker Communication](#13-worker-communication)
14. [API Design Philosophy](#14-api-design-philosophy)
...
| Role | Description |
|---|---|
| Student | Creates sessions, triggers flows, views own session |
| Mentor | Views sessions of supervised teams, adds comments |
| Admin | Views all sessions, audit logs, metrics |
...
|---|---|---|---|
| POST /sessions | ✅ | ❌ | ❌ |
| GET /sessions/{id} | ✅ (own only) | ✅ (team only) | ✅ |
| GET /sessions | ✅ (own only) | ❌ | ✅ |
| DELETE /sessions/{id} | ✅ (archive only) | ❌ | ✅ |
| POST /sessions/{id}/trigger/* | ✅ (own, correct state) | ❌ | ❌ |
| GET /sessions/{id}/history | ✅ (own) | ✅ (team) | ✅ |
...
| GET /audit | ❌ | ❌ | ✅ |
| GET /metrics | ❌ | ❌ | ✅ |
| POST /api/v1/internal/* | ❌ | ❌ | Workers (internal network only) |

## 8.3 Resource-Level Authorization

Role check alone is insufficient. A student with the `student` role must not access another team's session. After the role check, every data-fetching operation applies a **resource ownership filter**:

- Students: `query.filter(student_id == current_user.user_id)`
- Mentors: `query.filter(team_id IN current_user.supervised_team_ids)`
- Admins: no filter

This is enforced in the repository layer, not the route layer.
...
## 9.2 Embedding vs Referencing Strategy
...
## 10.1 Why a Repository Layer Exists
...
## 10.3 Idempotency in Repositories
...
## 11.3 Transaction-Like Behavior
...
## 12.4 Duplicate Publish Prevention
...
## 14.4 Request ID and Correlation ID
...
## 14.6 Standard Error Response Shape
...
## 16.4 Audit Log vs Application Log
...
- All query filters use indexed fields (see Section 9.3).
...
## 20.9 Concurrency and Race Conditions
...
## 21.5 OpenTelemetry Distributed Tracing
...
## 21.6 SAGA Pattern for Multi-Step Flows
...
## 21.7 CQRS (Command Query Responsibility Segregation)
